...
(function () {
  ...
  const profile = {
    ...
    async search({ query = '', filters = {}, limit = 30, from = 0 } = {}) { await requireUser(); let request = client.from('profiles').select('id, full_name, username, date_of_birth, gender, city, state, profession, education, religion, interests, languages, bio, manglik_status, avatar_url, is_verified, is_online, last_active_at, profile_media(*)').order('last_active_at', { ascending: false }).range(from, from + limit - 1); const term = query.trim().replace(/[,%()]/g, ''); if (term) request = request.or(`full_name.ilike.%${term}%,username.ilike.%${term}%,city.ilike.%${term}%,state.ilike.%${term}%,profession.ilike.%${term}%,education.ilike.%${term}%,religion.ilike.%${term}%`); if (filters.gender) request = request.eq('gender', filters.gender); if (filters.religion) request = request.eq('religion', filters.religion); if (filters.manglik) request = request.eq('manglik_status', filters.manglik); if (filters.profession) request = request.ilike('profession', `%${filters.profession}%`); if (filters.education) request = request.ilike('education', `%${filters.education}%`); if (filters.income) request = request.eq('income', filters.income); if (filters.verifiedOnly) request = request.eq('is_verified', true); if (filters.onlineOnly) request = request.eq('is_online', true); if (filters.recentlyActive) request = request.gte('last_active_at', new Date(Date.now() - 30 * 86400000).toISOString()); if (filters.ageMin || filters.ageMax) { const today = new Date(); if (filters.ageMin) { const latest = new Date(today.getFullYear() - Number(filters.ageMin), today.getMonth(), today.getDate()).toISOString().slice(0, 10); request = request.lte('date_of_birth', latest); } if (filters.ageMax) { const earliest = new Date(today.getFullYear() - Number(filters.ageMax) - 1, today.getMonth(), today.getDate() + 1).toISOString().slice(0, 10); request = request.gte('date_of_birth', earliest); } } return run(request, 'search profiles'); },
    async upload(file, type, sortOrder = 0) { const user = await requireUser(); if (!file?.type?.startsWith('image/')) throw new Error('Choose an image file.'); if (file.size > 8 * 1024 * 1024) throw new Error('Choose an image smaller than 8 MB.'); const bucketId = ({ avatar: 'avatars', cover: 'covers', gallery: 'gallery' })[type] || bucket; const extension = file.name.split('.').pop()?.toLowerCase() || 'jpg'; const path = `${user.id}/${crypto.randomUUID()}.${extension}`; await run(client.storage.from(bucketId).upload(path, file, { upsert: false, contentType: file.type }), 'upload image'); const row = await run(client.from('profile_media').insert({ profile_id: user.id, bucket_id: bucketId, storage_path: path, media_type: type, mime_type: file.type, size_bytes: file.size, sort_order: sortOrder }).select().single(), 'save media'); const { data } = await client.storage.from(bucketId).createSignedUrl(path, 60 * 60); if (type === 'avatar' || type === 'cover') await run(client.from('profiles').update({ [`${type}_url`]: path }).eq('id', user.id), 'link image'); return { ...row, signedUrl: data?.signedUrl || '' }; },
    async mediaUrls(media) { if (!media?.length) return []; return Promise.all(media.map(async (item) => ({ ...item, url: await storage.signedUrl(item.bucket_id || bucket, item.storage_path) }))); },
    async deleteMedia(media) { await requireUser(); await run(client.storage.from(media.bucket_id || bucket).remove([media.storage_path]), 'delete image'); return run(client.from('profile_media').delete().eq('id', media.id), 'delete media record'); },
  ...
  const social = {
    ...
    async saved() { const user = await requireUser(); return run(client.from('saved_profiles').select('created_at, profiles(*)').eq('user_id', user.id).order('created_at', { ascending: false }), 'load saved profiles'); },
    async matches() { const user = await requireUser(); return run(client.from('matches').select('*, user_one:profiles!matches_user_one_profile_fkey(*), user_two:profiles!matches_user_two_profile_fkey(*)').or(`user_one_id.eq.${user.id},user_two_id.eq.${user.id}`).order('updated_at', { ascending: false }), 'load matches'); }
  };
  const chat = {
    async conversations() { const user = await requireUser(); return run(client.from('conversation_members').select('conversation_id, is_favorite, last_read_at, conversations(id, updated_at, conversation_members(user_id, profiles!conversation_members_profile_fkey(full_name, username, avatar_url, is_online)))').eq('user_id', user.id).order('joined_at', { ascending: false }), 'load conversations'); },
    async messages(conversationId) { await requireUser(); return run(client.from('messages').select('*, sender:profiles!messages_sender_profile_fkey(full_name, avatar_url)').eq('conversation_id', conversationId).is('deleted_at', null).order('created_at'), 'load messages'); },
  ...
  const notifications = {
    async list() { const user = await requireUser(); return run(client.from('notifications').select('*, actor:profiles!notifications_actor_id_fkey(full_name, avatar_url)').eq('user_id', user.id).order('created_at', { ascending: false }), 'load notifications'); },
  ...
  const authApi = {
    ...
    async emailSignUp({ email, password, fullName, username }) { return run(client.auth.signUp({ email, password, options: { data: { full_name: fullName, username }, emailRedirectTo: window.location.origin } }), 'create account'); },
  ...
  const storage = {
    bucketFor(type) { return ({ avatar: 'avatars', cover: 'covers', gallery: 'gallery', document: 'documents', attachment: 'documents' })[type] || 'documents'; },
    async upload(file, type) { const user = await requireUser(); if (!file || file.size > 10 * 1024 * 1024) throw new Error('Choose a file smaller than 10 MB.'); const bucketId = this.bucketFor(type); const ext = file.name.split('.').pop()?.toLowerCase() || 'bin'; const path = `${user.id}/${crypto.randomUUID()}.${ext}`; await run(client.storage.from(bucketId).upload(path, file, { contentType: file.type, upsert: false }), 'upload file'); return { bucketId, path, fileName: file.name, mimeType: file.type, sizeBytes: file.size }; },
    async signedUrl(bucketId, path, expiresIn = 3600) { const { data, error } = await client.storage.from(bucketId).createSignedUrl(path, expiresIn); if (error) throw fail(error, 'create file preview'); return data.signedUrl; },
    async remove(bucketId, path) { return run(client.storage.from(bucketId).remove([path]), 'delete file'); }
  };
  const realtime = {
  ...
  const feed = {
    async list({ limit = 20 } = {}) { await requireUser(); return run(client.from('community_posts').select('*, profiles!community_posts_author_profile_fkey(full_name, username, avatar_url), post_reactions(user_id)').eq('is_published', true).order('created_at', { ascending: false }).limit(limit), 'load posts'); },
